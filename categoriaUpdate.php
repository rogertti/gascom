<?php
    ...
    $msg = "Campo obrigat&oacute;rio vazio.";
        ...
        if(empty($_POST['idcategoria'])) { die("reload"); }
        ...
        if($filtro == 2) {
            try {
                ...
                $sql = $pdo->prepare("SELECT idcategoria,monitor FROM categoria WHERE descricao = :descricao AND idcategoria <> :idcategoria");
                $sql->bindParam(':descricao', $_POST['descricao'], PDO::PARAM_STR);
                $sql->bindParam(':idcategoria', $_POST['idcategoria'], PDO::PARAM_INT);
                $sql->execute();
                $ret = $sql->rowCount();
                ...
                //atualiza no banco
                $sql = $pdo->prepare("UPDATE categoria SET descricao = :descricao,tipo = :tipo WHERE idcategoria = :idcategoria");
                $sql->bindParam(':descricao', $_POST['descricao'], PDO::PARAM_STR);
