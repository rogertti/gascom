...
(function ($) {
    ...
    $(".table-data").on('click', '.a-paid-lancamento', function (e) {
        e.preventDefault();
        
        var click = this.id.split('-'),
            py = click[0],
            id = click[1];
        location.href = 'lancamentoPaid.php?' + py + '=' + id;
        /*$.smkConfirm({
            text: 'O lançamento foi pago/recebido?',
            accept: 'Sim',
            cancel: 'Não'
        }, function (res) {
            if (res) {
    ...
    $(".table-data").on('click', '.a-delete-lancamento', function (e) {
        e.preventDefault();
        ...
        $.smkConfirm({
            text: 'Quer mesmo excluir o lançamento?',
            accept: 'Sim',
            cancel: 'Não'
        }, function (res) {
            if (res) {
                location.href = 'lancamentoDelete.php?' + py + '=' + id;
            }
        });
    });
    ...
    $(".form-new-categoria").submit(function (e) {
        ...
        $.post("categoriaInsert.php", { descricao: $("#descricao").val(), tipo: $("input[name='tipo']:checked").val(), rand: Math.random()}, function (data) {
            $(".btn-new-categoria").html('<img src="img/rings.svg" class="loader-svg">').fadeTo(fade, 1);
            ...
            case 'true':
                $.smkAlert({text: 'Categoria cadastrada com sucesso.', type: 'success', time: 2});
            ...
            $(".btn-new-categoria").html('Salvar').fadeTo(fade, 1);
        });
    ...
    $(".table-data").on('click', '.a-delete-categoria', function (e) {
        e.preventDefault();
        ...
        $.smkConfirm({
            text: 'Quer mesmo excluir a categoria?',
            accept: 'Sim',
            cancel: 'Não'
        }, function (res) {
            if (res) {
                location.href = 'categoriaDelete.php?' + py + '=' + id;
            }
        });
    });
    ...
    $(".form-new-usuario").submit(function (e) {
        ...
        $.post("usuarioInsert.php", { nome: $("#nome").val(), usuario: usuario, senha: senha, email: $("#email").val(), rand: Math.random()}, function (data) {
    ...
    // filtro por período
     ...
     $(".form-inline").on('click', '.btn-rangedate-report-categoria', function (e) {
        e.preventDefault();
        
        var periodo = $("#periodo").val(),
            click, date1, date2,
            py1 = '89d903bc35dede724fd52c51437ff5fd',
            py2 = 'ff9cf2d690d888cb337f6bf4526b6130';
        
            if(periodo == '') {
                $.smkAlert({text: 'O per&iacute;odo est&aacute; vazio.', type: 'warning', time: 3});
            } else {
                click = periodo.split(' - ');
                date1 = click[0];
                date2 = click[1];
                
                document.location = 'inicio?' + py1 + '=' + date1 + '&' + py2 + '=' + date2;
            }
    });
}(jQuery));